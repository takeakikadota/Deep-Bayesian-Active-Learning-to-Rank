...
# Modified for Bayesian DenseNet with Monte Carlo Dropout.
# Dropout layers remain active during inference by setting training=True.
...
BASE_WEIGTHS_PATH = ('https://storage.googleapis.com/tensorflow/''keras-applications/densenet/')
...
def dense_block(x, blocks, name, dropout_rate, weight_decay):
  """A dense block.
  ...
  for i in range(blocks):
    x = conv_block(x, 32, dropout_rate=dropout_rate, weight_decay=weight_decay, name=name + '_block' + str(i + 1))
  return x


def transition_block(x, reduction, name, dropout_rate, weight_decay):
  """A transition block.
...
def conv_block(x, growth_rate, name, dropout_rate, weight_decay):
  ...
  x1 = Dropout(dropout_rate)(x1, training=True)
...
def DenseNet(
  ...
  x = dense_block(x, blocks[0], dropout_rate=dropout_rate, weight_decay=weight_decay, name='conv2')
  x = transition_block(x, 0.5, dropout_rate=dropout_rate, weight_decay=weight_decay, name='pool2')
  x = dense_block(x, blocks[1], dropout_rate=dropout_rate, weight_decay=weight_decay, name='conv3')
  x = transition_block(x, 0.5, dropout_rate=dropout_rate, weight_decay=weight_decay, name='pool3')
  x = dense_block(x, blocks[2], dropout_rate=dropout_rate, weight_decay=weight_decay, name='conv4')
  x = transition_block(x, 0.5, dropout_rate=dropout_rate, weight_decay=weight_decay, name='pool4')
  x = dense_block(x, blocks[3], dropout_rate=dropout_rate, weight_decay=weight_decay, name='conv5')
  ...
  if include_top:
    ...
    x = layers.Dense(classes, activation=classifier_activation, kernel_regularizer=regularizers.l2(weight_decay), name='predictions')(x)
  else:
    if pooling == 'avg':
      x = layers.GlobalAveragePooling2D(name='avg_pool')(x)
    elif pooling == 'max':
...
def DenseNet121(include_top=True,
  ...
  return DenseNet([6, 12, 24, 16], include_top, weights, input_tensor,
                  input_shape, pooling, classes, dropout_rate, weight_decay)
